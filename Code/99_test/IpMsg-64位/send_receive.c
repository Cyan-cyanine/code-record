...
void* sendMsg(command* com)
{
  ...
  printf("发送对象：命令码=%#x 对象IP=%s\n",com->commandNo,inet_ntoa(com->peer.sin_addr));len = strlen(buf);
  printf("send buf : %s\n",buf);
}
...
int saySth(void)
{
  ...
  //显示在线用户列表---把每一个用户的信息放入到数组pusers里面
  ...
  if (who>0)
  {
    //因为数组下标从0开始，而用户列表从1开始。
    //当选中第12个用户时，实际上是选中了数组中的下标11的元素
    cur = pusers[who-1];

	//创建结构体，命令码为 发送信息+传送确认
    ...
    while(1)
    {
      ...
      //如果buf[0]不是回车，说明有输入数据，把数据保存到 com.additional里面
      if (buf[0] != '\n')
      {
...
void* getData(void* option)
{
  ...
  while((cur=head->next) != NULL)
  {
    ...
    if (tmp<0)
    {
      ...
      sendMsg(&com);
      pthread_mutex_lock(&getFMutex);
      gList->cancelled = 1;//取消该gsnode节点文件传输的标志位
      gList->transferring = 0;//文件停止传输
      pthread_mutex_unlock(&getFMutex);
      printf("\n%s fails to be transfered.\n", cur->fileName);
      return NULL;
    }
...
int getFile(void* option, gsNode *gList)
{
  ...
  initCommand(&com, IPMSG_GETFILEDATA);  //接收普通文件具体内容的命令码
  ...
  //组合字符串，把com节点中的信息按照一帧数据的方式写入buf缓冲区
  msgCreater(buf, &com, sizeof(buf));
  ...
  //每次从网上读取readBytes个数据，就向文件中写入 readBytes 个数据
  //每次写入后 filesize = filesize-readBytes.  剩下的字节数 = 总文件字节数-已写入字节数
  //当剩下的字节数==0时，跳出循环
  while (fileSize>0)
  {
...
int login(void)
{
  ...
  initCommand(&com, IPMSG_BR_ENTRY);//广播命令
  
  com.peer.sin_family = AF_INET;
  com.peer.sin_port = htons(IPMSG_DEFAULT_PORT);

  //把“255.255.255.255”放入到 com.peer.sin_addr 
